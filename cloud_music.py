...
def get_list(song_name):
    rt_list = []
    data = {"hlpretag": "<span class=\"s-fc7\">", "hlposttag": "</span>", "s": song_name, "type": "1", "offset": "0",
            "total": "true", "limit": "30", "csrf_token": ""}
    data_str = json.dumps(data)
    random_param_ = get_random()
    param_ = get_final_param(data_str, random_param_)
    song_list_ = get_music_list(param_['params'], param_['encSecKey'])
    if len(song_list_) > 0:
        song_list_ = json.loads(song_list_)['result']['songs']

        for i, item in enumerate(song_list_):
            item = json.dumps(item)
            data_str = {"ids": "[" + str(json.loads(str(item))['id']) + "]", "level": "standard", "encodeType": "",
                        "csrf_token": ""}
            data_str = json.dumps(data_str)
            param_ = get_final_param(data_str, random_param_)
            song_info = get_reply(param_['params'], param_['encSecKey'])
            if len(song_info) > 0:
                song_info = json.loads(song_info)
                song_url = json.dumps(song_info['data'][0]['url'], ensure_ascii=False)
                author = ' '.join([j["name"] for j in json.loads(item)["ar"]])

                rt_list.append({"name": json.loads(str(item))['name'], "author": author, "id": json.loads(item)["id"],
                                "url": song_url})
    else:
        return None
    return rt_list
