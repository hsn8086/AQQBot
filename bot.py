...
from util import CreateImg, is_cmd, is_txt, msg_chain_join, get_cmd
...
permissions_manager = Permissions('permissions.json')
    ...
    if is_cmd(cmd_str):
        ...
        if len(cmd_list) >= 1:
            if os.path.exists(os.path.join('commands', cmd_list[0] + '.py')):
                ...
                await app.send_message(user, CreateImg('无效的命令,请使用/help获取帮助'))
